...
会话描述协议定义于 [RFC 4566](https://tools.ietf.org/html/rfc4566) 中。它是一个key/value协议，每一行是一个值。看起来类似于INI文件。
...
WebRTC并未使用会话描述协议定义的所有key。您当前只需要理解下面的7个key。
...
这个列表包含了您将在WebRTC代理的会话描述中看到的一些常见属性。这些值控制着我们尚未讨论到的子系统。
