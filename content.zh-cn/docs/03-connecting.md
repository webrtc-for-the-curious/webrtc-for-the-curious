...
大多数应用程序使用客户端/服务器连接。在此场景下设置时，可以认为服务器具有稳定且公开可用的传输地址。客户端联系服务器，服务器响应。WebRTC不使用客户端/服务器进行连接，而是使用点对点（P2P）。在P2P连接中，创建连接的任务在两个peer之间几乎是平均分配的。之所以这样，是因为我们不能假定传输地址（IP和端口），且传输地址在会话期间甚至可能会变更。WebRTC将获取传输地址，并将尽力实现两个WebRTC代理之间的双向通信。
...
上面描述的连接过程是由 [ICE](https://tools.ietf.org/html/rfc8445) 实现的。这是另一个在WebRTC之前就已经出现的协议。
...
ICE就是克服现实世界网络限制的方法。在我们开始讨论ICE如何解决问题之前，先讨论一下有哪些问题。
...
但是，使用`Router B`的主机无法直接访问`Router A`背后的任何主机。您如何区分路由器A后面的191.168.0.1主机和路由器B后面相同IP的主机之间的区别呢？它们都使用内网IP！使用`Router B`的主机可以将数据直接发送到`Router A`，但是请求在那里就结束了。`Router A`怎么知道它应该将消息发送给哪台主机呢？

### 协议限制

有些网络不允许UDP通信，或者也有可能不允许TCP。有些网络的MTU非常低。网络管理员可以更改许多变量，这些修改可能会使通信变得困难。
...
NAT（网络地址转换）映射是使得WebRTC连接成为可能的魔法。WebRTC就是使用NAT让处于完全不同的子网中的两个peer进行通信，从而解决了上述"不在同一网络中"的问题，这同时也带来了新的挑战。首先，让我们解释一下NAT映射。
...
想要这样通信的话，您需要创建一个NAT映射。Agent 1使用端口7000与Agent 2建立WebRTC连接。这将创建一个192.168.0.1:7000到5.0.0.1:7000的绑定。然后，Agent 2就可以通过将数据包发送到5.0.0.1:7000，使得数据包到达Agent 1。在这个例子中，创建一个NAT映射，就像是在路由器中做了一次自动化且无需配置的端口转发。
...
让我们从对STUN的基本描述开始。稍后，我们再将话题扩展到TURN和ICE的用法。现在，我们只打算描述请求/响应流程来创建映射。然后，我们讨论如何获取该映射的详细信息以便与他人共享。当您在ICE网址中有一个用于WebRTC PeerConnection的`stun:`服务器时，此过程就会发生。简而言之，STUN向NAT外部的STUN服务器发送请求，服务器返回其在请求中观察到的内容，STUN根据这些内容来帮助NAT后面的端点找出创建的映射。
...
[RFC5780](https://tools.ietf.org/html/rfc5780)定义了一种方法，可以运行一个测试来确定您的NAT类型。 这很有用，因为您可能会提前知道是否可以进行直接连接。

## TURN

在无法建立直接连接的情况下，[RFC5766](https://tools.ietf.org/html/rfc5766)中定义了TURN（使用中继穿透NAT）。当您的两个peer的NAT类型不兼容，或者双方使用不同协议时，就需要使用TURN！对于保护隐私的需求而言，TURN也很重要。如果通过TURN运行所有通讯，客户的真实地址在对端是被隐藏的。

TURN使用专用服务器。该服务器充当客户端的代理。客户端连接到TURN服务器并创建一个对应的`Allocation`。通过创建该`Allocation`，客户端将获得一个临时`IP/端口/协议`三元组，其他peer通过该`IP/端口/协议`可以将数据发送给该客户端。这个新的监听地址被称为`中继传输地址`。您可将其视为转发地址并分享给他人，以便其他人可以通过TURN向您发送流量！对于每个将获得该`中继传输地址`的peer，您必须为其创建一个`Permission`，以允许它与您进行通信。
...
`Allocations`是TURN的核心。本质上，一个`allocation`就是一个`TURN会话`。要创建一个TURN allocation，您需要与TURN `Server Transport Address`（服务器传输地址）（通常在3478端口）进行通信。
...
在您为远程主机创建权限之前，远程主机是无法通过您的`中继传输地址`发送数据的。所谓创建权限，即是告诉TURN服务器一个IP/端口，TURN服务器将允许发该IP/端口发送入站流量。
...
对于上面那种错误情况，假设您要为一个使用`地址相关的映射`的NAT网络的主机创建权限，如果您从其他TURN服务器生成`映射地址`，则所有入站流量都将被丢弃。因为每次他们与其他主机通信时，它都会生成一个新的映射。
...
ICE（交互式连接建立）是WebRTC连接两个代理的方式。这也是WebRTC之前的一项技术，在[RFC8445](https://tools.ietf.org/html/rfc8445)中定义！ICE是用于建立连接的协议。它会确定两个peer之间所有可能的路由，然后确保您保持连接状态。
...
每一方都必须有一个`用户片段`和`密码`。必须先交换这两个值，才能进行连接检查。`用户片段`以纯文本形式发送，用于多个ICE会话的解复用（demux）。
...
如果您与代理位于同一网络中，则可以通过多播找到彼此。如果不在同一网络中，则将无法连接（除非网络管理员配置为允许多播数据包通过）。
...
`控制中`的代理和`受控中`的代理都开始在每个候选地址对上发送流量数据。这样是必须的，因为如果一个代理位于`地址相关映射`网络中，这样会创建`Peer自反候选地址`。

每个收到流量数据的`候选地址对`，会被提升为`有效候选地址`对。接下来，`控制中`的代理将指定一个`有效候选地址`对。这就是`指定候选地址对`。然后，`控制中`的代理和`受控中`的代理再尝试进行一轮双向通信。如果成功，则`指定候选地址对`将成为`选定的候选地址对`！它将被用于后面的会话中。

### 重新启动

如果`选定的候选地址对`由于任何原因停止工作（如：NAT映射到期，TURN服务器崩溃等），则ICE代理将进入`失败`状态。此时可以重新启动两个代理，然后重新执行整个过程。