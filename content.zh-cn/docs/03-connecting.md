...
在无法建立直接连接的情况下，[RFC 5766](https://tools.ietf.org/html/rfc5766)中定义了TURN（使用中继穿透NAT）。当您的两个peer的NAT类型不兼容，或者双方使用不同协议时，就需要使用TURN！对于保护隐私的需求而言，TURN也很重要。如果通过TURN运行所有通讯，客户的真实地址在对端是被隐藏的。
