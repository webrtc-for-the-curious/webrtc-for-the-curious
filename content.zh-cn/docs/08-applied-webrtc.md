...
许多人认为WebRTC只是在浏览器中实现电话会议的技术。实际上它能做的不仅如此！
WebRTC被广泛用于各种用例。新的用例一直在出现。这里是一些常见的用例，以及WebRTC是如何对它们进行革新的。

### 电话会议

电话会议是WebRTC的原始用例。该协议包含浏览器中几个必要功能，这些功能没有其他协议提供支持。您可以使用WebSockets构建会议系统，并且在最好的条件下，它是可以工作的。但如果您希望在现实世界的网络条件下部署一些服务，那么WebRTC是最佳选择。
...
#### 网格（Mesh）
我们经常希望将两个以上的WebRTC peer连接在一起，比如在Web群组聊天或在线多人游戏中。要在两个以上的peer之间实现WebRTC通信的方式也很多。一种选择是创建点对点网格。
点对点网格有两种形式，"全网格"和"部分网格"。在"全网格"中，每个peer都跟要通信的每个其他peer建立一条单独的WebRTC连接。在"部分网格"中，某些peer比其他peer分配更多的连接，并用于在peer之间转发和/或路由信息。无论哪种情况，每个peer都必须将媒体的副本发送到它所连接的每个其他peer，因此在有许多用户连接时，每个peer都需要付出很高的带宽成本。考虑到这些带宽问题，点对点网格只适合用于连接在一起的小型群组。

#### 混合网格（Hybrid Mesh）

### 客户端-服务器
WebRTC协议的低延迟特性非常适合通话，通常会看到以p2p网状配置（低延迟）安排会议，或者通过SFU（选择性转发单元）进行对等通信以提高通话质量。由于编解码器支持因浏览器而异，因此许多会议服务器允许浏览器使用的专有或收费的编解码器（例如h264等）进行广播，然后在服务器级别将其重新编码为VP8等开放标准。如果SFU不仅转发数据包，还要执行编码任务的话，我们现在将它称为MCU（多点会议单元）。
尽管众所周知，SFU既快速高效又非常适合召开会议，但MCU可能会占用大量资源！一些会议服务器甚至会执行更繁重的任务，例如为每个呼叫者合成（合并）定制的A/V流，这样来自所有其他呼叫者的流会被合并成一个，以最大程度地减少客户端带宽的使用。
