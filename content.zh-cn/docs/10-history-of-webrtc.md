...
在学习 WebRTC 时，开发人员经常对复杂性感到沮丧，他们希望WebRTC能够更简单一些，他们总能看到其中很多与他们项目无关的功能特性。但是问题在于，不同的开发者有迥然不同的应用场景。实时通信有着一段丰富的历史，人们在这个领域创造过很多不同的东西。 

我们知道WebRTC使用了很多现有的协议，本章记录有对这些协议作者的采访。这些协议作者的采访可以让我们深入了解每个协议在构建过程中所作的设计。
最后会有一个关于WebRTC本身的采访，采访对象是一位WebRTC的初创成员。
当你了解一个软件的意图和设计时，你就能更有效地使用它。
...
WebRTC 的标准化 比 本章提到的其他工作都要复杂的多。它需要两个不同领域的标准组织（IETF 和 W3C），需要来自许多国家和公司的人们共同完成。[Serge Lachapelle](https://twitter.com/slac) 会跟我们谈谈， 为什么要开发 WebRTC，以及开发过程中所付出的大量努力。

Serge 是 Google 的产品经理，目前担任Google Workspace的产品经理 。这是我对采访的总结。

### 什么促使你参与 WebRTC ？

从大学开始，我就一直热衷于构建通信软件。 90年代像[nv](https://github.com/ronf/nv)这样的技术开始出现，但很难使用。我创建了一个项目，它允许你可以直接从浏览器加入视频通话。我也将它移植到 Windows。

我把这段经历带到了我共同创立的公司 Marratech。我们构建了用于群组视频会议的软件。我们的产品从技术上来讲是非常不同，当时的视频通信的前沿是使用多播网络。
用户可以依靠网络将视频数据包传送到通话中的每一个人。这意味着我们的服务器可以非常简单。但是存在一个很大的缺点，网络必须设计成能适配多播模式。然后行业开始了一场技术变革：从多播网络，到 混洗交换网络 （packet shufflers），即 SFUs .

Marratech 于 2007 年被 Google 收购。然后我得以继续这个后来成为 WebRTC的项目。

### 在Google的第一个项目

WebRTC 前身团队(*后来才有WebRTC项目*) 参与的第一个项目是 Gmail 语音和视频聊天。 将音频和视频导入浏览器并不容易。 我们需要从各个公司获得 多个功能模块的 许可。音频由 GIP 授权，视频由 Vidyo 授权， 网络组件由 libjingle 授权，然后我们要变魔术一般地将这些组件组合在一起，让它们能够很好的相互协作。

每个子系统都有完全不同的 API，而且都是为了解决不同的问题而设计的。 为了让这一切能够有机的结合，你需要同时具备多个领域的专业知识：网络、密码学、流媒体等。
[Justin Uberti](https://juberti.com) 是这项工作的负责人。他将这些组件组合在一起，开发了一个可用的产品。

在浏览器中做实时渲染也非常困难。 在当时我们不得不使用NPAPI（Netscape Plugin API），并做了很多创新性的工作，最终得以实现 浏览器上的实时渲染。
我们从这个项目中学到的很多经验教训，极大地影响了后面的 WebRTC。

### Chrome

与此同时，Chrome 项目在 Google 内部启动。 Chrome在项目之初就拥有远大的格局，带来了很多令人兴奋的东西，诸如 WebGL、离线(Offline)、数据库功能、游戏低延迟输入 等， 创新数不胜数。

是否摒弃 NPAPI 是当时的一个争论焦点。 它是一个强大的 API，但也引入了诸多的安全性问题。 Chrome 使用沙盒设计来确保用户安全。用户的任务被隔离在各自的进程中运行，所有可能存在的安全问题也随之得到隔离。即使出现一些问题，攻击者也无法访问用户数据。

### WebRTC 诞生

我的个人理解是，多方面的因素和诉求，最终促成了 WebRTC 的诞生。

构建 实时通信软件不应该如此艰难 。开发者们耗费大量的精力来实现相同的东西。我们应该一次性解决这些 实时通信领域的 通用问题，然后就专注于其他事情。

人际沟通应该是畅通无阻的，应该是开放的。为什么 文本和 HTML 可以在浏览器里打开，而我的实时音视频却不能？

安全是重中之重。使用 NPAPI 对用户来说并不是最好的。这也促使考虑，我们要制定的协议，应该默认就是安全的，而不需要额外的手段来保证安全。

为了实现 WebRTC，Google 收购并开源了很多我们用到过的组件，包括 [On2](https://en.wikipedia.org/wiki/On2_Technologies) 的视频技术 和
[Global IP Solution](https://en.wikipedia.org/wiki/Global_IP_Solutions) 的 RTC 技术。其中，我负责 GIPS 的收购工作。我们必须将这些结合起来，让他们在浏览器内外都能够简单易用。

### 标准化

标准化 WebRTC 是我们想做却一直没有做的事情。 这一点上，我们很幸运，Google 有 Harald Alvestrand 的加入。 他此前已经在 IETF 做过很多工作, 并且 启动了 WebRTC 标准化 进程。

2010 年夏天，在马斯特里赫特安排了一次非正式午餐。 各个公司的开发者们齐聚一堂，讨论 WebRTC 应该是什么样的。 餐桌上有来自 Google、Cisco、Ericsson、Skype、Mozilla、Linden Labs 等公司的工程师。
你可以在以下位置找到完整的出席和演示者幻灯片[rtc-web.alvestrand.com](http://rtc-web.alvestrand.com)。

Skype 同样提供了一些很好的指导意见，因为他们那时已经在IETF完成了 Opus的标准化工作，比较有经验。

### 站在巨人的肩膀上

参与IEFT的标准化工作，远远不像开发WebRTC，会遇到很多前所未有的事情。对WebRTC而言，我们很幸运有很多已经现有技术可以使用。我们不需要关注所有的问题，因为很多问题已经有现有的技术可以解决。如果你不喜欢现有技术，就很麻烦了。除非很有必要，不然抛弃现有技术去重新造轮子一般都不是好选择。

我们也有意识地避免重新标准化某些领域，比如 信令（signaling），信令问题 已经通过 SIP 和其他非 IETF 等工作得到解决，重新标准化这些领域，最终可能会变得非常政治化。 还有最本质的原因是，我们觉得并没有对这些领域创造什么有价值的贡献。

我没有像 Justin 和 Harald 那样全程参与标准化工作，不过参与其中的这段时间我也很开心。 当然，重新回来为用户创造产品 让我更兴奋。

### 未来

WebRTC 今天正处在一个很好的时代。创新迭代在这个领域不断发生，而且这些创新并没有局限于我们曾经做过的一切。

我最兴奋的是云计算将会如何影响通信技术。 使用高级算法，我们可以从通话中去除背景噪音，我们也可以在以前没法通信的地方实现通信。 我们看到 WebRTC也不再局限于通信……谁能想到它已经被应用于云游戏9年之久了呢？ 如果不是 基于 WebRTC ，这一切都很难做到。