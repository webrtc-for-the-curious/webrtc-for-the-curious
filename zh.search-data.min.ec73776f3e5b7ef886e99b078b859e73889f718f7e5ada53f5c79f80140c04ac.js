"use strict";(function(){const t={encode:!1,tokenize:function(e){return e.replace(/[\x00-\x7F]/g,"").split("")}};t.doc={id:"id",field:["title","content"],store:["title","href","section"]};const e=FlexSearch.create("balance",t);window.bookSearchIndex=e,e.add({id:0,href:"/zh/docs/01-what-why-and-how/",title:"是什么，为什么，如何使用",section:"Docs",content:` 是什么，为什么，如何使用 # WebRTC 是什么？ # WebRTC 是 Web 实时通信（Real-Time Communication）的缩写，它既是 API 也是协议。WebRTC 协议是两个 WebRTC Agent 协商双向安全实时通信的一组规则。开发人员可以通过 WebRTC API 使用 WebRTC 协议。目前 WebRTC API 仅有 JavaScript 版本。
...
`}),e.add({id:1,href:"/zh/docs/02-signaling/",title:"信令",section:"Docs",content:` 信令 # 什么是 WebRTC 信令？ # 当一个 WebRTC Agent 被创建时，它对其他 peer 一无所知。它不知道它将与谁联系，也不知道它们将发送些什么！ 信令是使呼叫成为可能的初始引导程序。交换信令消息后，WebRTC Agent 才可以直接相互通信。
...
重新协商（Renegotiation） 同步广播（Simulcast） `}),e.add({id:2,href:"/zh/docs/03-connecting/",title:"连接",section:"Docs",content:` 连接 # 为什么 WebRTC 需要专用的子系统进行连接？ # 目前，大多数部署的应用程序都通过客户端 / 服务器方式进行连接。客户端 / 服务器方式连接要求服务器具有稳定且公开可用的传输地址。客户端与服务器联系，然后服务器做出响应。
...
Peer 自反 # 当远端 Peer 从其之前未知的地址收到本端发出的请求时，远端 Peer 将会报告（反射）他看到的本端地址给我们，这个地址就是本端的 Peer 自反候选地址。由于 ICE 是经过身份验证的协议，因此远端 Peer 知道是本端而非其他人在与他通信。
...
`}),e.add({id:3,href:"/zh/docs/04-securing/",title:"安全性",section:"Docs",content:` 安全性 # WebRTC 具有哪些安全性保障？ # 每个 WebRTC 连接都经过身份验证和加密。你可以确信第三方看不到你发送的内容，也无法插入虚假消息。你还可以确保与你进行通信的 WebRTC Agent 正是生成会话描述的 Agent。
...
`}),e.add({id:4,href:"/zh/docs/05-real-time-networking/",title:"搭建实时网络",section:"Docs",content:` 实时网络 # 为什么网络在实时通信中如此重要？ # 网络是实时通信中的限制因素。在理想的世界中，我们将拥有无限的带宽，并且数据包会即时到达。但事实并非如此。网络是受限的，且其限定条件随时可能更改。测量和观察网络状况也是一个难题。根据你所使用的硬件，软件及其配置，你可能看到不同的表现。
...
`}),e.add({id:5,href:"/zh/docs/06-media-communication/",title:"媒体通信",section:"Docs",content:` 媒体通信 # 我可以从 WebRTC 的媒体通信中得到什么？ # WebRTC 允许你发送和接收无限多条音频和视频流。你可以在通话期间随时添加和删除这些流。这些流可以全部独立，也可以捆绑在一起！你甚至可以将网络摄像头的音频和视频放到你桌面的视频中，然后将此视频以 feed 的形式发送出去。
...
GCC 有几种替代方案，例如
NADA: A Unified Congestion Control Scheme forReal-Time Media SCReAM - 自时钟多媒体速率适应。 `}),e.add({id:6,href:"/zh/docs/07-data-communication/",title:"数据通信",section:"Docs",content:` 数据通信 # 我可以从 WebRTC 的数据通信中获得什么？ # WebRTC 提供用于数据通信的数据通道。在两个 peer 之间，你可以打开 65,534 个数据通道。 数据通道基于数据报，并且每个通道都有其自己的持久性设置。默认设置下，每个数据通道都能保证有序交付。
...
`}),e.add({id:7,href:"/zh/docs/08-applied-webrtc/",title:"WebRTC 应用场景",section:"Docs",content:` WebRTC 应用场景 # 现在你已经知道 WebRTC 的工作原理，到了使用它的时候了！本章探讨人们使用 WebRTC 构建什么以及他们是如何实现的。你将学到基于 WebRTC 发生的所有有趣的事情。WebRTC 的功能是有代价的。建立产品级的 WebRTC 服务相当有挑战性。本章将尝试解释这些挑战性的根源，这样你遇到问题时就能有所准备。
...
多点会议单元（Multi-point Conferencing Unit） # MCU（多点会议单元）与 SFU 的客户端 / 服务器拓扑类似，但它会对输出流进行组合。MCU 将要出站的媒体重新编码为一个聚合流，而不是直接分发未经修改的版本。 `}),e.add({id:8,href:"/zh/docs/09-debugging/",title:"调试",section:"Docs",content:` 调试 # 调试 WebRTC 可能是一项艰巨的任务。有很多部分都处于运行状态，每一个部分都可能出现问题。如果你不够细心，可能会浪费数周的时间来查看错误的模块。当你最终找到出错的部分时，你还需要学习一些知识才能理解问题的根源。
...
`}),e.add({id:9,href:"/zh/docs/10-history-of-webrtc/",title:"历史",section:"Docs",content:` 历史 # 在学习 WebRTC 时，开发人员经常对其复杂性感到沮丧，他们认为一些 WebRTC 功能与他们当前的项目无关，并希望 WebRTC 能够更简单一些。但问题是不同的开发者有迥然不同的应用场景。实时通信有着一段丰富的历史，人们在这个领域创造过很多不同的东西。
...
`}),e.add({id:10,href:"/zh/docs/11-faq/",title:"常见问题",section:"Docs",content:` 常见问题 # 为什么 WebRTC 使用 UDP？ NAT 穿透需要 UDP。没有 NAT 穿透，就无法建立 P2P 连接。UDP 不像 TCP 那样 &quot; 保证送达 &ldquo;，因此 WebRTC 在用户级别提供这一特性。
要了解更多信息，请参考 连接 章节。
